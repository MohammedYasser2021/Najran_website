...
const MainNavbar = ({ currentLang, changeLanguage }: MainNavbarProps) => {
  ...
  return (
    <>
      <nav className="bg-gray-50 shadow-md border-b border-gray-200 py-4 font-['Cairo'] sticky top-0 z-40 w-full">
            ...
            <div className={isArabic ? "order-1" : "order-3"}>
              <div className="relative w-72">
                <input
                  ...
                  className={`w-full bg-white border border-gray-200 py-3 rounded-full text-sm focus:outline-none focus:border-[#1389bf] transition-all
                    ${isArabic ? 'pr-11 pl-5 text-right' : 'pl-11 pr-5 text-left'}`}
                />
                <Search
                  className={`absolute top-1/2 -translate-y-1/2 text-gray-400 text-lg
              ...
              >
                {menuItems.map((item) => (
                  <li key={item.key} className="relative group">
                    {item.hasDropdown ? (
                      <button className="flex items-center gap-1.5 py-3 px-4 hover:text-[#1389bf] transition-colors whitespace-nowrap">
                        {item.label}
                        <ChevronDown className="text-sm transition-transform duration-300 group-hover:rotate-180" size={16} />
                      </button>
                    ) : (
                      <a
                        href={item.href}
                        className="py-3 px-4 hover:text-[#1389bf] transition-colors block whitespace-nowrap"
                      >
                        {item.label}
                      </a>
                    )}
                    {item.hasDropdown && (
                      <div
                        className={`absolute top-full mt-2 bg-white shadow-xl rounded-2xl py-4 px-5 w-64 z-50
                        ...
                        <div className="flex flex-col gap-1 text-sm">
                          {item.items?.map((sub, i) => (
                            <a
                              key={i}
                              href={sub.href}
                              className="px-5 py-3 hover:bg-gray-50 hover:text-[#1389bf] rounded-xl transition-all"
                            >
                              {sub.label}
                            </a>
                          ))}
            ...
            {isArabic ? (
              <>
                <button
                  ...
                  className="text-gray-700 hover:text-[#1389bf] transition-colors"
                >
                  <Menu size={28} />
                </button>
                <a href="tel:920002159" className="flex items-center gap-2 text-gray-700 hover:text-[#1389bf] transition-colors">
                  <Phone size={20} />
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 ${isArabic ? 'left-0' : 'right-0'} h-full w-80 bg-black text-white z-50 transform transition-transform duration-300 ease-out md:hidden overflow-y-auto
        ...
        <div className="p-6">
                ...
                {item.hasDropdown ? (
                  <>
                    <button
                      ...
                      className={`w-full flex items-center justify-between py-2 px-4 hover:bg-gray-800 rounded-lg transition-all ${isArabic ? 'text-right' : 'text-left'}`}
                    >
                      ...
                      <ul className={`py-1 ${isArabic ? 'pr-4' : 'pl-4'} space-y-0.5`}>
                        {item.items?.map((sub, i) => (
                          <li key={i}>
                            <a
                              href={sub.href}
                              className="block py-1.5 px-4 hover:bg-gray-800 rounded-lg text-xs text-gray-300 hover:text-white transition-all"
                            >
                              {sub.label}
                            </a>
                          </li>
                  ...
                  <a
                    href={item.href}
                    className={`block py-2 px-4 hover:bg-gray-800 rounded-lg font-medium text-sm transition-all ${isArabic ? 'text-right' : 'text-left'}`}
                  >
                    {item.label}
                  </a>
                )}
          ...
          <div className="mb-6">
            <div className="relative">
              <input
                ...
                className={`w-full bg-gray-800 border border-gray-700 py-2 rounded-lg text-sm text-white placeholder-gray-400 focus:outline-none focus:border-[#1389bf] transition-all
                  ${isArabic ? 'pr-11 pl-5 text-right' : 'pl-11 pr-5 text-left'}`}
              />
              <Search
          ...
          <div className="space-y-3 mb-6 border-t border-gray-700 pt-4">
            <a href="mailto:info@hospital.com" className={`flex items-center gap-3 text-gray-300 hover:text-white transition-colors ${isArabic ? 'flex-row-reverse text-right' : ''}`}>
              ...
              <span className="text-xs">info@hospital.com</span>
            </a>
            <a href="#" className={`flex items-center gap-3 text-gray-300 hover:text-white transition-colors ${isArabic ? 'flex-row-reverse text-right' : ''}`}>
              ...
              <span className="text-xs">{isArabic ? 'القاهرة، مصر' : 'Cairo, Egypt'}</span>
            </a>
            <a href="tel:920002159" className={`flex items-center gap-3 text-gray-300 hover:text-white transition-colors ${isArabic ? 'flex-row-reverse text-right' : ''}`}>
              ...
              <span className="text-xs">{isArabic ? '920002159' : '920002159'}</span>
            </a>
          </div>
  ...
  <div className="flex items-center justify-center gap-3">
    ...
    >
      ...
      {isShareOpen && (
        ...
        >
          <div
            className="bg-white rounded-3xl p-8 w-full max-w-sm mx-4 shadow-2xl animate-[slideUp_0.3s_ease-out]"
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-8">
              <a href={`https://wa.me/?text=${encodeURIComponent(shareText + " " + shareUrl)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center group">
                <div className="w-16 h-16 flex items-center justify-center rounded-2xl bg-[#25D366] text-white text-3xl font-bold shadow-md group-hover:scale-110 transition-transform duration-300">
                  W
                </div>
                <span className="text-xs font-medium mt-3 text-gray-700">{isArabic ? 'واتساب' : 'WhatsApp'}</span>
              </a>
              <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center group">
                <div className="w-16 h-16 flex items-center justify-center rounded-2xl bg-black text-white text-3xl font-bold shadow-md group-hover:scale-110 transition-transform duration-300">
                  𝕏
                </div>
                <span className="text-xs font-medium mt-3 text-gray-700">{isArabic ? 'تويتر' : 'Twitter'}</span>
              </a>
              <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center group">
                <div className="w-16 h-16 flex items-center justify-center rounded-2xl bg-[#1877F2] text-white text-3xl font-bold shadow-md group-hover:scale-110 transition-transform duration-300">
                  f
                </div>
                <span className="text-xs font-medium mt-3 text-gray-700">{isArabic ? 'فيسبوك' : 'Facebook'}</span>
              </a>
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            <div className="text-center">
              <Globe className="text-5xl text-[#1389bf] mx-auto mb-4" size={48} />
